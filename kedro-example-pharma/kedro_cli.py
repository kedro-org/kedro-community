...
def run(tag, env, parallel, runner, node_names, to_nodes, from_nodes):
    """Run the pipeline."""
    from kedro_example_pharma.run import main
...
def build_docs():
    ...
    python_call(
        "ipykernel", ["install", "--user", "--name=kedro_example_pharma"]
    )
...
def convert_notebook(all_flag, overwrite_flag, filepath):
    ...
    *Note*: Make sure your notebooks have unique names!
    ...
    """
    from kedro_example_pharma.run import ProjectContext
    ...
    kedro_package_name = "kedro_example_pharma"
